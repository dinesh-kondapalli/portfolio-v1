// biome-ignore-all lint: Skiper UI registry (third-party)
"use client";
...
export const useThemeToggle = ({
  ...
  /** Serialize View Transitions so a new one never starts while another is active (avoids AbortError). */
  const viewTransitionChainRef = useRef(Promise.resolve());
  const runViewTransition = useCallback((switchTheme: () => void) => {
    if (typeof document === "undefined" || !document.startViewTransition) {
      switchTheme();
      return;
    }
    viewTransitionChainRef.current = viewTransitionChainRef.current
      .catch(() => {
        /* keep queue alive after skip/abort */
      })
      .then(() => {
        try {
